...
function displayTasks (filter) {
  ...
  if(gorevListesi.length == 0){
    ul.innerHTML = "<p class='p-3 m-0 text-center'>Görev listeniz boş.</p>"
  }else{
    for (let gorev of gorevListesi) {
      ...
      let li = `
        <li class="task list-group-item">
          <div class="chechbox">
            <input type="checkbox" onclick="updateStatus(this)" id="${gorev.id + 1}" class="form-check-input" ${completed} />
            <label for="${gorev.id + 1}" class="form-check-label ${completed}">${
